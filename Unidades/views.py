...
def editarUnidades(request, id_unidad):
    ...
    if request.method == 'POST':
        ...
        if salida == 1:
            ...
            data['mensaje'] = 'No se ha podido Editar la unidad ' + \
                str(id_unidad)
            messages.info(request, 'Operación Fallida!')
...
def editarUnidad(id_unidad, nombre_departamento, id_cliente):
    ...
    cursor.callproc("SP_MODIFICAR_UNIDAD", [
                    id_unidad, nombre_departamento, id_cliente, salida])
...
def borrarUnidad(request, id_unidad):
    ...
    if request.method == 'POST':
        ...
        if salida == 1:
            ...
            data['mensaje'] = 'No se ha podido eliminar la unidad ' + \
                str(id_unidad)
            messages.info(request, "Operación Fallida")
...
def agregar_unidad(nombre_departamento, id_cliente):
    ...
    cursor = django_cursor.connection.cursor()
    salida = cursor.var(cx_Oracle.NUMBER)
    cursor.callproc('SP_CREAR_UNIDAD', [
                    nombre_departamento, id_cliente, salida])
    return salida.getvalue()
